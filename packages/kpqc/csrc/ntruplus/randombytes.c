...
//
// RNG shim for the WebAssembly build of NTRU+.
//
// NTRU+'s reference code calls randombytes() for all secret randomness, and the
// KAT harness additionally calls randombytes_init() to drive a deterministic
// NIST AES-256 CTR-DRBG (so the official Known Answer Tests can be reproduced).
// This shim provides BOTH sources and lets the JS wrapper switch between them:
//
//   * Secure mode (default): randombytes() defers to ntruplus_js_random_fill(),
...
//
// The DRBG below is the standard NIST AES-256 CTR-DRBG (identical to the one in
// the reference kat/rng.c), implemented with the portable AES from the vendored
// NTRU+ kat/aes.c so the KAT bytes match exactly. This file replaces the
// upstream randombytes.c in the wasm build.
...
extern void ntruplus_js_random_fill(unsigned char *x, size_t xlen);
...
// keygen/encapsulation use.
EMSCRIPTEN_KEEPALIVE
void ntruplus_use_secure_rng(void) { g_use_drbg = 0; }

void randombytes(uint8_t *out, size_t outlen) {
  if (!g_use_drbg) {
    ntruplus_js_random_fill(out, outlen);
    return;
  }
