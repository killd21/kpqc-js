...
//
// RNG shim for the WebAssembly build of SMAUG-T.
//
// SMAUG-T's reference code calls randombytes() for all secret randomness, and
...
//
//   * Secure mode (default): randombytes() defers to smaugt_js_random_fill(),
...
extern void smaugt_js_random_fill(unsigned char *x, size_t xlen);
...
EMSCRIPTEN_KEEPALIVE
void smaugt_use_secure_rng(void) { g_use_drbg = 0; }

int randombytes(uint8_t *out, size_t outlen) {
  if (!g_use_drbg) {
    smaugt_js_random_fill(out, outlen);
    return 0;
  }
