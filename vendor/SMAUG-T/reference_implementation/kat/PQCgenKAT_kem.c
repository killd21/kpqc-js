...
int main(int argc, const char **argv) {
    ...
    unsigned char ct[CRYPTO_CIPHERTEXTBYTES], ss[SMAUGT_CRYPTO_BYTES],
        ss1[SMAUGT_CRYPTO_BYTES];
    int count;
    unsigned char pk[CRYPTO_PUBLICKEYBYTES], sk[CRYPTO_KEM_SECRETKEYBYTES];
    int ret_val;

    sprintf(fn_rsp, "PQCkemKAT_%s.rsp", CRYPTO_ALGNAME);
    if ((fp_rsp = fopen(fn_rsp, "w")) == NULL) {
        printf("Couldn't open <%s> for write\n", fn_rsp);
        return KAT_FILE_OPEN_ERROR;
    }

    sprintf(fn_req, "PQCkemKAT_%s.req", CRYPTO_ALGNAME);
    ...
    while (1) {
        ...
        if ((ret_val = kat_crypto_kem_keypair(pk, sk, d, seed)) != 0) {
        ...
        if ((ret_val = kat_crypto_kem_enc(ct, ss, pk, mu)) != 0) {
            printf("crypto_kem_enc returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        fprintBstr(fp_rsp, "ct = ", ct, CRYPTO_CIPHERTEXTBYTES);
        fprintBstr(fp_rsp, "ss = ", ss, SMAUGT_CRYPTO_BYTES);
        ...
        if (memcmp(ss, ss1, SMAUGT_CRYPTO_BYTES)) {
